...
import com.weibo.dip.hadoop.yarn.scheduler.fair.SchedulingPolicy;
...
public class DominantResourceFairnessPolicy extends SchedulingPolicy {
	...
	public static class DominantResourceFairnessComparator implements Comparator<Schedulable> {
		 ...
		 * vectors. The shares vector contains, for each resource, the fraction
		 * of the pool that it takes up. The resourceOrder vector contains an
		 * ordering of resources by largest share. So if resource=<10 MB, 5
		 * CPU>, and pool=<100 MB, 10 CPU>, shares will be [.1, .5] and
		 * resourceOrder will be [CPU, MEMORY].
		 */
		void calculateShares(Resource resource, Resource pool, ResourceWeights shares, ResourceType[] resourceOrder,
				ResourceWeights weights) {
			shares.setWeight(MEMORY, (float) resource.getMemory() / (pool.getMemory() * weights.getWeight(MEMORY)));
			shares.setWeight(CPU,
					(float) resource.getVirtualCores() / (pool.getVirtualCores() * weights.getWeight(CPU)));
		...
		private int compareShares(ResourceWeights shares1, ResourceWeights shares2, ResourceType[] resourceOrder1,
				ResourceType[] resourceOrder2) {
