...
/**
 * Event log used by the fair scheduler for machine-readable debug info. This
 * class uses a log4j rolling file appender to write the log, but uses a custom
 * tab-separated event format of the form:
 * 
 * <pre>
 * DATE    EVENT_TYPE   PARAM_1   PARAM_2   ...
 * </pre>
 * 
 * Various event types are used by the fair scheduler. The purpose of logging in
 * this format is to enable tools to parse the history log easily and read
 ...
 *
 * Constructing this class creates a disabled log. It must be initialized using
 * {@link FairSchedulerEventLog#init(Configuration, String)} to begin writing to
 * the file.
 */
@Private
@Unstable
class FairSchedulerEventLog {
	...
	/**
	 * Log directory, set by mapred.fairscheduler.eventlog.location in conf
	 * file; defaults to {hadoop.log.dir}/fairscheduler.
	 */
