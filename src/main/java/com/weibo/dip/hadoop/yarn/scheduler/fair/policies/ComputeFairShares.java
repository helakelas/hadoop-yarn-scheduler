 ...
 */
package com.weibo.dip.hadoop.yarn.scheduler.fair.policies;
...
import com.weibo.dip.hadoop.yarn.scheduler.fair.FSQueue;
import com.weibo.dip.hadoop.yarn.scheduler.fair.Schedulable;
...
public class ComputeFairShares {
	...
	/**
	 * Compute fair share of the given schedulables.Fair share is an allocation
	 * of shares considering only active schedulables ie schedulables which have
	 * running apps.
	 * 
	 * @param schedulables
	...
	/**
	 * Compute the steady fair share of the given queues. The steady fair share
	 * is an allocation of shares considering all queues, i.e., active and
	 * inactive.
	 *
	 * @param queues
	 * @param totalResources
	 ...
	 * fair shares. The min and max shares and of the Schedulables are assumed
	 * to be set beforehand. We compute the fairest possible allocation of
	 * shares to the Schedulables that respects their min and max shares.
	 * 
	 * To understand what this method does, we must first define what weighted
	 * fair sharing means in the presence of min and max shares. If there were
	 * no minimum or maximum shares, then weighted fair sharing would be
	 ...
	 * being fair if there exists a ratio R such that: Schedulables S where
	 * S.minShare > R * S.weight are given share S.minShare - Schedulables S
	 * where S.maxShare < R * S.weight are given S.maxShare - All other
	 * Schedulables S are assigned share R * S.weight - The sum of all the
	 ...
	 * 
	 * We compute a fair allocation by finding a suitable weight-to-slot ratio
	 * R. To do this, we use binary search. Given a ratio R, we compute the
	 * number of slots that would be used in total with this ratio (the sum of
	 * the shares computed using the conditions above). If this number of slots
	 * is less than totalSlots, then R is too small and more slots could be
	 * assigned. If the number of slots is more than totalSlots, then R is too
	 * large.
	 * 
	 ...
	 * computed to be large enough that too many slots are given (by doubling R
	 * until we use more than totalResources resources). The helper method
	 ...
	 * Schedulables, because resourceUsedWithWeightToResourceRatio is
	 * linear-time and the number of iterations of binary search is a constant
	 * (dependent on desired precision).
	 */
	 ...
	 * resources taken by fixed fairshare schedulables, and adds the remaining
	 * to the passed nonFixedSchedulables.
	 */
