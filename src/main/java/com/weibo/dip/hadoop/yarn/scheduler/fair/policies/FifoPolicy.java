...
public class FifoPolicy extends SchedulingPolicy {
	...
	static class FifoComparator implements Comparator<Schedulable>, Serializable {
		...
		public int compare(Schedulable s1, Schedulable s2) {
			...
			if (res == 0) {
				// In the rare case where jobs were submitted at the exact same
				// time,
				// compare them by name (which will be the JobID) to get a
				// deterministic
				// ordering, so we don't alternately launch tasks from different
				// jobs.
	...
	public boolean checkIfUsageOverFairShare(Resource usage, Resource fairShare) {
		throw new UnsupportedOperationException("FifoPolicy doesn't support checkIfUsageOverFairshare operation, "
