...
public class QueueManager {
	...
	public void initialize(Configuration conf)
			throws IOException, SAXException, AllocationConfigurationException, ParserConfigurationException {
	 ...
	 * necessary. If the queue is not or can not be a leaf queue, i.e. it
	 * already exists as a parent queue, or one of the parents in its name is
	 * already a leaf queue, null is returned.
	 * 
	 ...
	 * is necessary. If the queue is not or can not be a parent queue, i.e. it
	 * already exists as a leaf queue, or one of the parents in its name is
	 * already a leaf queue, null is returned.
	 * 
	 * The root part of the name is optional, so a queue underneath the root
	 * named "queue1" could be referred to as just "queue1", and a queue named
	...
	/**
	 * Make way for the given queue if possible, by removing incompatible queues
	 * with no apps in them. Incompatibility could be due to (1) queueToCreate
	 * being currently a parent but needs to change to leaf (2) queueToCreate
	 * being currently a leaf but needs to change to parent (3) an existing leaf
	 * queue in the ancestry of queueToCreate.
	 * 
	...
	private boolean removeEmptyIncompatibleQueues(String queueToCreate, FSQueueType queueType) {
		...
		// Ensure queueToCreate is not root and doesn't have the default queue
		// in its
		// ancestry.
		if (queueToCreate.equals(ROOT_QUEUE)
				|| queueToCreate.startsWith(ROOT_QUEUE + "." + YarnConfiguration.DEFAULT_QUEUE_NAME + ".")) {
			return false;
		}
	...
	public void updateAllocationConfiguration(AllocationConfiguration queueConf) {
		...
		rootQueue.recomputeSteadyShares();
		// Update the fair share preemption timeouts and preemption for all
		// queues
		// recursively
