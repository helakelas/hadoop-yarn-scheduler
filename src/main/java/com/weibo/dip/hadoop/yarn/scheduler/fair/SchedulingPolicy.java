...
import com.weibo.dip.hadoop.yarn.scheduler.fair.policies.DominantResourceFairnessPolicy;
import com.weibo.dip.hadoop.yarn.scheduler.fair.policies.FairSharePolicy;
import com.weibo.dip.hadoop.yarn.scheduler.fair.policies.FifoPolicy;

@Public
@Evolving
public abstract class SchedulingPolicy {
	private static final ConcurrentHashMap<Class<? extends SchedulingPolicy>, SchedulingPolicy> instances = new ConcurrentHashMap<Class<? extends SchedulingPolicy>, SchedulingPolicy>();
	 ...
	 * DominantResourceFairnessPolicy. For a custom {@link SchedulingPolicy}s in
	 * the RM classpath, the policy should be canonical class name of the
	 ...
	 * at the specified depth in the hierarchy
	 * 
	 * @param policy
	 ...
	 * {@link SchedulingPolicy}, to be used later for scheduling decisions. The
	 * shares computed are instantaneous and only consider queues with running
	 * applications.
	 * 
	 * @param schedulables
	 ...
	 */
	public abstract void computeShares(Collection<? extends Schedulable> schedulables, Resource totalResources);

	/**
	 * Computes and updates the steady shares of {@link FSQueue}s as per the
	 * {@link SchedulingPolicy}. The steady share does not differentiate between
	 * queues with and without running applications under them. The steady share
	 * is not used for scheduling, it is displayed on the Web UI for better
	 * visibility.
	 *
	 * @param queues
	 *            {@link FSQueue}s whose shares are to be updated
	 * @param totalResources
	 *            Total {@link Resource}s in the cluster
	 */
	public abstract void computeSteadyShares(Collection<? extends FSQueue> queues, Resource totalResources);

	/**
	 * Check if the resource usage is over the fair share under this policy
	 *
	 * @param usage
	 *            {@link Resource} the resource usage
	 * @param fairShare
	 *            {@link Resource} the fair share
	...
	/**
	 * Check if a leaf queue's AM resource usage over its limit under this
	 * policy
	 *
	 * @param usage
	 *            {@link Resource} the resource used by application masters
	 * @param maxAMResource
	 *            {@link Resource} the maximum allowed resource for application
	 *            masters
	 * @return true if AM resource usage is over the limit
	 */
	public abstract boolean checkIfAMResourceUsageOverLimit(Resource usage, Resource maxAMResource);

	/**
	 * Get headroom by calculating the min of <code>clusterAvailable</code> and
	 * (<code>queueFairShare</code> - <code>queueUsage</code>) resources that
	 * are applicable to this policy. For eg if only memory then leave other
	 * resources such as CPU to same as clusterAvailable.
	 *
	 * @param queueFairShare
	 *            fairshare in the queue
	 * @param queueUsage
	 *            resources used in the queue
	 * @param clusterAvailable
	 *            available resource in cluster
	 * @return calculated headroom
	 */
	public abstract Resource getHeadroom(Resource queueFairShare, Resource queueUsage, Resource clusterAvailable);
