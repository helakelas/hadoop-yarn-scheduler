...
public class FairSharePolicy extends SchedulingPolicy {
	 ...
	 * are as a ratio. For example, if job A has 8 out of a min share of 10
	 * tasks and job B has 50 out of a min share of 100, then job B is scheduled
	 * next, because B is at 50% of its min share and A is at 80% of its min
	 * share.
	 * 
	 * Schedulables above their min share are compared by (runningTasks /
	 * weight). If all weights are equal, slots are given to the job with the
	 * fewest tasks; otherwise, jobs with more weight get proportionally more
	 * slots.
	 */
	private static class FairShareComparator implements Comparator<Schedulable>, Serializable {
		...
		public int compare(Schedulable s1, Schedulable s2) {
			...
			useToWeightRatio1 = s1.getResourceUsage().getMemory() / s1.getWeights().getWeight(ResourceType.MEMORY);
