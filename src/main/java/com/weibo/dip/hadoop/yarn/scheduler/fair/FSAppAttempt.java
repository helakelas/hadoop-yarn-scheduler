...
public class FSAppAttempt extends SchedulerApplicationAttempt implements Schedulable {
	 ...
	 * opportunities since the last container was scheduled at the current
	 * allowed level and the time since the last container was scheduled.
	 * Currently we use only the former.
	 */
	...
	public Resource getHeadroom() {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Headroom calculation for " + this.getName() + ":" + "Min(" + "(queueFairShare=" + queueFairShare
					...
					+ "(clusterResource=" + clusterResource + " - clusterUsage=" + clusterUsage + ")" + "Headroom="
					+ headroom);
		}
		return headroom;
	}
	...
	/**
	 * Return the level at which we are allowed to schedule containers, given
	 * the current size of the cluster and thresholds indicating how many nodes
	 * to fail at (as a fraction of cluster size) before relaxing scheduling
	 * constraints.
	 */
	...
	/**
	 * Return the level at which we are allowed to schedule containers. Given
	 * the thresholds indicating how much time passed before relaxing scheduling
	 * constraints.
	 */
	...
	/**
	 * Reserve a spot for {@code container} on this {@code node}. If the
	 * container is {@code alreadyReserved} on the node, simply update relevant
	 * bookeeping. This dispatches ro relevant handlers in
	 * {@link FSSchedulerNode}..
	 */
	 ...
	 * does not have enough memory, create a reservation. This is called once we
	 * are sure the particular request should be facilitated by this node.
	 *
	 * @param node
	 ...
	 * @param reserved
	 *            Whether there's already a container reserved for this app on
	 *            the node.
	...
	private Resource assignContainer(FSSchedulerNode node, boolean reserved) {
		...
		Collection<Priority> prioritiesToTry = (reserved)
				? Arrays.asList(node.getReservedContainer().getReservedPriority()) : getPriorities();
